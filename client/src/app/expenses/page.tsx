...
import { ExpenseByCategorySummary, useGetExpenseByCategorySummaryQuery } from "@/state/api";
...
const Expense = () => {
  ...
  const { data: expenseSummary, isLoading, isError } = useGetExpenseByCategorySummaryQuery();
  const expenses = useMemo(() => expenseSummary ?? [], [expenseSummary]);
  ...
  const classNames = {
    ...
    selectInput:
      "mt-1 block w-full pl-3 py-2 text-base border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md",
  };

  if (isLoading) return <div>Loading...</div>;

  if (!expenseSummary || isError) return <div>Fail to fetch expense</div>;
